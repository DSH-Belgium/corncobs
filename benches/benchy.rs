...
pub fn criterion_benchmark(c: &mut Criterion) {
    let mut group = c.benchmark_group("encode_random_1024");
    let mut out = [0; corncobs::max_encoded_len(RANDOM_1024.len())];
    group.bench_function("encode_buf random 1024", move |b| b.iter(|| {
        corncobs::encode_buf(black_box(&RANDOM_1024), &mut out);
    }));

    let mut out = [0; corncobs::max_encoded_len(RANDOM_1024.len())];
    group.bench_function("encode_iter random 1024", move |b| b.iter(|| {
        for (b, o) in corncobs::encode_iter(black_box(&RANDOM_1024)).zip(&mut out) {
            *o = b;
        }
    }));

    let mut buffer = RANDOM_1024.to_vec();
    buffer.resize(corncobs::max_encoded_len(RANDOM_1024.len()), 0);
    group.bench_function("encode_in_place random 1024", move |b| b.iter_batched(
            || buffer.clone(),
            |mut buffer| corncobs::encode_in_place(black_box(&mut buffer), RANDOM_1024.len()),
